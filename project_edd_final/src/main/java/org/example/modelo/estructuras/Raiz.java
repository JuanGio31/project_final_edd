...
public class Raiz {
    ...
    public void setTengoHijos(NodoArbolB nodo) {
        if (nodo == primerNodo) {
    ...
    public void ingresarEnHijos(NodoArbolB conHijos, int valor) {
        boolean entro = false;
    ...
    public void ubicarValorEnArreglo(NodoArbolB nodoA, int valor) {
        int cont = 0;
        while (cont <= 2 * grado) {
    ...
    public void ordenarNodos(NodoArbolB aOrdenar) {
        int i, j;
        i = 0;
        NodoArbolB tmp;
    ...
    public void split(NodoArbolB nodo) {

        NodoArbolB hijoIzq = new NodoArbolB();
        NodoArbolB hijoDer = new NodoArbolB();
    ...
    public void eliminar(int valor) throws ListaException { //elimina de la lista el valor y vuelve a crear el arbol
        boolean encontrado = false;
        int j = 0;
        for (int i = 0; i < Lista.ingresados.getSize() && !encontrado; i++) {
            if (Lista.ingresados.obtenerElemento(i) == valor) {
                encontrado = true;
                j = i;
            }
        }
        if (encontrado) {
            Lista.ingresados.eliminar(j);
        } else {
            System.out.println("El valor a eliminar no se encuentra en el arbol B");
        }
        Listado<Integer> auxiliar = Lista.ingresados;
        Lista.ingresados = new Listado<>();
        primerNodo = new NodoArbolB();
        primerNodo.tengoHijos = false;
        for (int k = 0; k < auxiliar.getSize(); k++) {
            Integer y = auxiliar.obtenerElemento(k);
            int o = y.intValue();
            insertar(o);
        }
    }
    ...
    public boolean buscar(int valor) throws ListaException {
        boolean esta = false;
        for (int i = 0; i < Lista.ingresados.getSize() && !esta; i++) {
            if (Lista.ingresados.obtenerElemento(i) == valor) {
                esta = true;
    ...
    public String recorrer(NodoArbolB nodo) {
        arbol += "\n";
